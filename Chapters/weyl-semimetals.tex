...
\begin{itemize}
	...
	\item This chapter contains no new results, its purpose is to provide the necessary background to our results in the next chapter.
	
	\item The first section reviews physical aspects of WSM, 
\end{itemize}
}

\section{Physical aspects}\label{sec:semimetal-physics}

%TODO Physics writing

{\color{blue}
\begin{itemize}
	\item The generic Hermitian two-band Hamiltonian in three dimensions is
	\begin{equation*}
	...
	\end{equation*}
	so that the gap between the two bands closes exactly when $\h(\k) = 0$. Since $\h(\k)$ has three components that all depend on three momentum degrees of freedom, these band crossings are expected to appear generically at discrete points in the Brillouin zone. These band crossings are ``accidental'' in the sense that they are not enforced by any symmetry of the system, but they are generally robust to perturbations.
	
	\item Near a band intersection at $\k=\k_0$, the Hamiltonian can be linearized: writing $\delta\k := \k - \k_0$, it can be expanded as
	...
	\end{equation}
	where $(\vb{v}_\mu)_i := \dee_{k^i}h_\mu |_{\k=\k_0}$ records the rate of change of the different components of the Hamiltonian. The traceless linear term containing the sigma matrices is of particular interest: it bears resemblance to (and in specific cases, reduces to) the Hamiltonians describing individual Weyl fermions:
	\begin{equation*}
		H_\pm = \mp \vb{p}\cdot\bm{\upsigma}.
	\end{equation*}
	This similarity is the reason the point $k_0$ is called a \emph{Weyl point} or \emph{Weyl node}. It hints at an important feature of these band crossings: like Weyl fermions, they have an intrinsic chirality associated with them. This chirality $\chi$ can be calculated in generic cases by collecting the velocities $\vb{v}$ into a $3\times 3$ matrix $V_{ij} := (\vb{v}_i)_j$ and calculating the sign of its determinant:
	\begin{equation*}
		\chi = \sign{|V_{ij}|},
	\end{equation*} 
	...
	\item Band crossings may occur between any two bands, but degeneracies in bands other than the conduction and valence bands generally do not affect the electronic properties of a material. Even between the valence and conduction bands, the different gap closings can occur at any energy in principle, determined by $h_0(\k)$. If this energy is sufficiently far from the Fermi level, the Fermi surface has an extensive two-dimensional structure, and normal metallic dispersive behaviour dominates. If $h_0(\k)$ is close enough to the Fermi level at all band intersections, the Fermi surface becomes point-like to good approximation, and the material is in a semimetallic phase. This is what is referred to as a (type I) Weyl semimetal.
	
	\item Small note on $\vb{v}_0$ and cone tilt/type II WSMs
	
	\item Nielsen--Ninomiya
	...
	\item Chiral anomaly cancelling $\leftrightarrow$ Nielsen--Ninomiya?
\end{itemize}
}
